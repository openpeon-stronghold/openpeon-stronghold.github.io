...
const briefingUrls = Array.from(
  { length: BRIEFING_FRAMES },
  (_, i) =>
    `assets/images/modal-frame-${String(i).padStart(2, "0")}.webp`,
);
...
function buildSoundList(slug, manifest) {
  ...
  for (const [catKey, catData] of Object.entries(
    manifest.categories || {},
  )) {
...
export async function openModal(slug, display, frameBg) {
  ...
  const frag = document
    .getElementById("modal-tpl")
    .content.cloneNode(true);
  frag.querySelector(".portrait-bg").style.backgroundImage =
    `url('${frameBg}')`;
  ...
  frag.querySelector(".modal-cmd-text").textContent =
    `peon packs install ${packName}`;
  ...
  body
    ...
    .addEventListener("click", () => {
      ...
      navigator.clipboard
        .writeText(body.querySelector(".modal-cmd-text").textContent)
        .catch(() => {});
      flashCopy(btn);
    });
  ...
  try {
    ...
    if (section)
      section.innerHTML =
        '<div class="modal-error">Sounds coming soon.</div>';
  }
}
